...
export async function submitFeedback({ predictionId, predictedPlan, correctPlan, rating, inputs }) {
  return _post('/feedback', { prediction_id: predictionId, predicted_plan: predictedPlan,
...
export function getMockRecommendation(inputs) {
  ...
  const allRanked = Object.entries(PLANS_MOCK).map(([id, p]) => {
    ...
    return { ...p, plan_id:id, match_score:parseFloat(score.toFixed(1)), recommended:id===pid,
  ...
  return {
    ...
    persona, xai:{feature_contributions:xai,top_driver:xai[0].label,
    ...
    recommendation:{plan_id:pid,...plan,confidence:conf,
      ...
      overpayment_analysis:{overpaying:diff>100,amount:Math.max(0,Math.round(diff)),
